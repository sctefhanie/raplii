...
if not isfolder("Raplii") then
    makefolder("Raplii")
end
if not isfolder("Raplii/Config") then
    makefolder("Raplii/Config")
end
...
local ConfigFile = "Raplii/Config/Chloe_" .. gameName .. ".json"
...
local Rapliiex = {}
function Rapliiex:MakeNotify(NotifyConfig)
    local NotifyConfig = NotifyConfig or {}
    NotifyConfig.Title = NotifyConfig.Title or "Raplii"
...
function Rapliiex(msg, delay, color, title, desc)
    return Rapliiex:MakeNotify({
        Title = title or "Raplii",
...
function Rapliiex:Window(GuiConfig)
    GuiConfig              = GuiConfig or {}
    GuiConfig.Title        = GuiConfig.Title or "Raplii"
    GuiConfig.Footer       = GuiConfig.Footer or "Rapli :3"
    GuiConfig.Color        = GuiConfig.Color or Color3.fromRGB(255, 0, 255)
    GuiConfig["Tab Width"] = GuiConfig["Tab Width"] or 100
    ...
    local Rapliix = Instance.new("ScreenGui");
    local DropShadowHolder = Instance.new("Frame");
    local DropShadow = Instance.new("ImageLabel");
    ...
    Rapliix.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    Rapliix.Name = "Rapliix"
    Rapliix.ResetOnSpawn = false
    Rapliix.Parent = game:GetService("CoreGui")
    ...
    DropShadowHolder.ZIndex = 0
    DropShadowHolder.Name = "DropShadowHolder"
    DropShadowHolder.Parent = Rapliix

    DropShadowHolder.Position = UDim2.new(0, (Rapliix.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0,
        (Rapliix.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
    ...
    function GuiFunc:DestroyGui()
        if CoreGui:FindFirstChild("Rapliix") then
            Rapliix:Destroy()
        end
    end
    ...
    Close.Activated:Connect(function()
        ...
        Yes.MouseButton1Click:Connect(function()
            if Rapliix then Rapliix:Destroy() end
